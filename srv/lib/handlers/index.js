...
module.exports = function (entities) {
	...
	this.before("CREATE","PLITEMS", async(Taxi) => {
		...
		const updId = await db.preparePromisified(
			`SELECT IFNULL(MAX(ITEM_ID),0) + 1 AS MAX_ID FROM PIPELIFE`
		);
		
		const updResults = await db.statementExecPromisified(updId);
		console.log(updResults[0].MAX_ID);
		data.ITEM_ID = updResults[0].MAX_ID;
	...
	// this.on("CREATE", "PLITEMS", async(Taxi) => {
	// 	const {
	// 		data
	// 	} = Taxi;
	// 	if (data.length < 1) {
	...
	// this.on("UPDATE", "PLITEMS", async(User) => {
	...
	// this.on("DELETE", "PLITEMS", async(User) => {
	// 	console.log("Delete");
	// 	const {
	// 		data
	// 	} = User;
